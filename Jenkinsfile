  ...
  stages {
   ...
   stage('s3-sites-upload') {
    ...
    stage('Deploy') {
      ...
      environment {
        // develop AND qa BRANCHES WILL BE DEPLOYED TO nonprod:TAG
        // release OR master BRANCH WILL BE DEPLOYED TO prod:TAG
        RANCHER_PROD_CLUSTER = "prod"
        RANCHER_NONPROD_CLUSTER = "nonprod"
        RANCHER_PROJECT = "TAG"

        // COMMA SEPARATED LIST OF SECRET RESOURCES NAMES TO VERIFY IN NAMESPACE ${BRANCH_NAME}-${IMAGE_NAME} BEFORE DOING A DEPLOY
        VERIFY_SECRETS="${IMAGE_NAME}"

        // TO DEPLOY DEVOPS UTILITY APP. ALL ENVIRONMENTS WILL BE DEPLOYED TO devops CLUSTER
        // RANCHER_PROD_CLUSTER = "devops"
        // RACNHER_NONPROD_CLUSTER = "devops"

        // OMIT CHART* VARIABLES IF DEPLOYING EXISTING YAMLs from ci/k8s/deploy/${BRANCH_NAME}/
        // IF CHART VARIABLE EXISTS, BUOY WILL FETCH CHART AND CREATE A YAML FROM CHART TO DEPLOY TO RANCHER/KUBERNETES
        // CHART = "nginx-custom-chart"
        // CHART_REPO = "helm-virtual"

        // OPTIONAL, OTHERWISE > latest
        //CHART_VERSION="0.1.0"

        // INCLUDE IF DEPLOYING OUR CUSTOM BUILT DOCKER IMAGES
        // THOSE IMAGES HAVE A UNIQUE TAG USED FOR KUBERNETES DEPLOYMENT IN ADDITION TO: develop, qa, release TAGS
        IS_IMAGE_UNIQUE_TAG="true"

        // ENVIRONMENT VARIABLES TO SUBSTITUTE IN YAML FILES. envsust runs on all files in deploy/ dir which may contain autogenerated yaml from chart
        // NOTE!!! <<< SINGEL QUOTES >>>>
        // IF ONE OF THE VARIBALES SUPPLIED IS EMPTY, BUOY ERRORS AND EXISTS.
        //ENVSUBST_VARIABLES='$UNIQUE_IMAGE_TAG:$TOMCAT_MEMORY:$NGINX_DIR:$SOME_OTHER'
        ENVSUBST_VARIABLES='$IMAGE_UNIQUE_TAG:$BRANCH_NAME:$IMAGE_NAME:$PROXY_NAME'

        // OPTIONAL:
        IS_ROLLBACK_ENABLED="true" // Default=true, HERE FOR SHOW PURPOSES. SET TO false TO DISABLE ROLLBACK ON FAILED ROLLOUT.
        //PROGRESS_DEADLINE_SECONDS="300" // Default=300, TIMEOUT PERIOD FOR ROLLOUT BEFORE CONSIDERING FAILED ROLLOUT TO INITIATE ROLLBACK.
        //MIN_READY_SECONDS="60" // Default=30, TIME PERIOD PODS SHOULD STAY READY FOR TO CONSIDER ROLLOUT SUCCESFULL.
        //REVISION_HISTORY_LIMIT="5" // Default=10. ROLLOUT VERSIONS TO KEEP

        // Enables liquibase Deploy. Default liquibase data dir is ci/db/scripts/
           // IS_LIQUIBASE=""                          // or "yes", must be not empty to enable liquibase deploy
           // LIQUIBASE_DB_USER_KEY=""               // LIQUIBASE_DB_USER_KEY="liquibase-db-user"
           // LIQUIBASE_DB_PASS_KEY=""           // LIQUIBASE_DB_PASS_KEY="liquibase-db-pass"
           // LIQUIBASE_DB_URL_KEY=""      // DEFAULT
           // LIQUIBASE_DATA_DIR=""           // DEFAULT
           // LIQUIBASE_ENTRY_SCRIPT="liquibase_deploy.sh" // DEFAULT
      }
      steps {
        withCredentials([
