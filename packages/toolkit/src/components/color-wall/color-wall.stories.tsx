...
const unChunkedChunksMap = (type): Color[][] => {
  const chunkNames: any =
    type === 'family' ? sections.find(({ name }) => name === 'Sherwin-Williams Colors').families : sections
...
const Template = (args): JSX.Element => {
  ...
  const hashesColorHex = (colors): Color[] =>
    colors.map((color) => ({ ...color, hex: `#${String(color.hex.replace(/#/g, ''))}` }))
    ...
    <ColorWall
      ...
      colors={
        args.colors && args.colors.length > 0
          ...
          : args.family
          ? getColors(unChunkedChunksMap('family')[args.family])
          : getColors(unChunkedChunksMap('section')[args.section])
      }
...
EmeraldColors.args = {
  ...
  gridWidth: 5,
  singleChunk: false,
  section: 'Emerald Designer Edition',
  wrappingEnabled: true
}

export const LivingWellColors = Template.bind({})
LivingWellColors.args = {
  chunkWidth: 4,
  gridWidth: 6,
  singleChunk: false,
  section: 'Living Well Colors',
  wrappingEnabled: true
}

export const SingleChunkEmeraldColors = Template.bind({})
SingleChunkEmeraldColors.args = {
  chunkWidth: 20,
  gridWidth: 1,
  singleChunk: true,
  section: 'Emerald Designer Edition',
...
WarmRedRedColors.args = {
  chunkWidth: 20,
  gridWidth: 1,
  sortBy: 'Lightness',
  orderBy: 'Ascending',
  colors: warmRedColors
}
...
export default {
  ...
  argTypes: {
    ...
    family: {
      control: { type: 'select' },
      description:
        'defines the family to display colors from<br /><em>(overrides section when defined)</em><br /> `storybook args only`',
    ...
    sortBy: {
      ...
      options: ['Hue', 'Saturation', 'Lightness', 'LRV']
    },
