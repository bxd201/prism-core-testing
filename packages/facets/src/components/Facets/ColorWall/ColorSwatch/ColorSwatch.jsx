...
export const Content = ({ msg, color, style }: ContentProps) => {
  ...
  const { brandId, brandKeyNumberSeparator, swatchShouldEmit, allowedAnalytics }: ConfigurationContextType =
    ...
    <div className='color-swatch__button-group' style={style}>
      {displayAddButton &&
        (colorIsInLivePalette ? (
          ...
          <button
            title={(addButtonText || at(messages, 'ADD_TO_PALETTE')[0] || '').replace(
              '{name}',
              fullColorName(color.brandKey, color.colorNumber, color.name, brandKeyNumberSeparator)
            )}
            onClick={() => {
              ...
              const gtmData = createGTMData(
                ...
                ANALYTICS_INTERACTIONS_TYPE.INTERACTION,
                null,
                'add color card'
              )
              gtmData.color_data = `${color.brandKey}${color.colorNumber}`.toLowerCase()
...
const ColorSwatch = React.forwardRef<ColorSwatchProps, HTMLElement>(({ color, contentRenderer = (defaultContent) => <>
      {defaultContent}
    </>, gap, isClickable = true, level, showContents = level === 0, status, style, onFocus, outline = true, className }: ColorSwatchProps, ref) => {
  const {
    url,
    ...
    <>
      <button
        className={`${baseClass} ${baseClass}-${
          level === undefined ? `flat${gap ? '-gap' : ''}` : numToAlphaString(level)
        }${
          !isClickable || (chunkClickable && section === kebabCase(primeColorWall)) ? ' color-swatch--no-outline' : ''
        }`}
        ...
        onFocus={onFocus}
        onClick={
          !isClickable || (chunkClickable && section === kebabCase(primeColorWall))
            ? noop
            : () => {
                GA.event(
                  {
      ...
      {showContents && (
        <section
          ...
          onFocus={onFocus}
          aria-label={fullColorName(color.brandKey, color.colorNumber, color.name, brandKeyNumberSeparator)}
          className={`${baseClass}__content${color.isDark ? ' color-swatch__content--dark-color' : ''}${
            outline ? ` ${baseClass}__content--focus` : ''
          }${houseShaped && level === 0 ? ' color-swatch--no-outline' : ''}`}
          ...
          {contentRenderer([
            colorNumOnBottom ? (
              <div key='0'>
                ...
                <p className={`${baseClass}__${colorNumOnBottomClass}__number`}>
                  {fullColorNumber(color.brandKey, color.colorNumber, brandKeyNumberSeparator)}
                </p>
              </div>
            ) : (
              <div key='0'>
                <p className='color-swatch__content__number'>
                  {fullColorNumber(color.brandKey, color.colorNumber, brandKeyNumberSeparator)}
                </p>
