...
export default (props: Props) => {
  ...
  const { brandId, cvw = {}, allowedAnalytics }: ConfigurationContextType = useContext(ConfigurationContext)
  const { palette = {} } = cvw
  ...
  const onDownloadClick = async () => {
    ...
    if (isPaintScene) {
      ...
      await getFlatImage(backgroundImg, paintLayer).then((url) => {
        imageSrc = url
      })
    }

    const variant = !isPaintScene
      ? variantsCollection.find(
    ...
    const sectionNameMapping = swatchColors.map(({ colorFamilyNames = [] }) => {
      ...
      return structure
        .filter((s) => s.families.indexOf(fam) > -1)
        .map((s) => s?.name ?? null)
    ...
    generateImage(data, colors, config, intl, swatchColors, sectionNameMapping)
      ...
      .then((buffer) => {
        ...
        reader.onloadend = () => {
          ...
          link.href = navigator.userAgent.match('CriOS') /* iOS Chrome */
            ? reader.result
    ...
    pushToDataLayer(
      createGTMData(
        ...
        isPaintScene ? ANALYTICS_LABELS.USER_UPLOAD : variant?.description
      ),
      allowedAnalytics
    )
  }
