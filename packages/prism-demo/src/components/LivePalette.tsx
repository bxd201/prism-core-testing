...
import { ColorsIcon, Color, LivePalette } from '@prism/toolkit'
...
export interface LivePaletteProps {
  colors: Color[]
  lpColors: Color[]
  setLpColors: (colors: Color[]) => Color[]
}

const LivePaletteWrapper = ({ colors, lpColors, setLpColors }: LivePaletteProps) => (
  <LivePalette
    addButtonRenderer={(colors) => (
      <button
        className='h-full'
        onClick={() => {
          console.log('"Add a Color" button triggered')
        }}
        style={{ width: 'inherit' }}
      >
    ...
    detailsButtonRenderer={({ coordinatingColors, name }) => (
      <button
        ...
        onClick={() => {
          console.log('Details button triggered')
        }}
      >
        <ColorsIcon
          className='w-5 h-5'
    ...
    onColorsChanged={(colors) => setLpColors(colors)}
  />
)
