...
export const fetchRemoteScenes = (
  brandId: string,
  options: any,
...
export const handleScenesFetchedForCVW = (data: ScenePayload) => {
  ...
  const sceneTypes = Object.keys(SCENE_TYPES)
    ...
    .filter((sceneType) => sceneType) // transform sparse array to a dense one

  const vectorizedScenes = sceneTypes.reduce((acc, curr) => {
    return [
      ...acc,
      data[curr].scenes.map((datum) => {
        return {
          id: datum.id,
          width: datum.width,
          height: datum.height,
...
// This method gets a flat array of variants these are referential to the scene they belong to.
