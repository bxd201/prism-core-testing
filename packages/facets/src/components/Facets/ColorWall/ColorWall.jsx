...
const ColorWall = ({ section: sectionOverride, family: familyOverride, colorId: colorIdOverride }: ColorWallProps) => {
  const {
    ...
    isChipLocator,
    leftHandDisplay,
    swatchMaxSize: globalSwatchMaxSize,
    swatchMinSize,
    swatchSizeZoomed,
  ...
  const {
    brandId,
    colorWall: { bloomEnabled = true, colorSwatch = {}, gapsBetweenChunks = true },
    uiStyle
  ...
  const { url, params: _params }: { url: string, params: { section: ?string, family?: ?string, colorId?: ?string } } =
    useRouteMatch()
  ...
  const {
    chunkGridParams = {},
    family,
    items: { colorMap = {}, colorStatuses = {}, sectionLabels: _sectionLabels = {} },
    primeColorWall,
    section = '',
    sectionsShortLabel,
    unChunkedChunks
  ...
  const swatchSizeUnzoomed: number = chunkGridParams.wrappingEnabled
    ? swatchMaxSize
    : clamp(
        containerWidth / (getWidthOfWidestChunkRowInChunkGrid(chunkGrid) + lengthOfLongestChunkRow + 1),
        swatchMinSize,
        swatchMaxSize
      )
    ...
    if (activeColorRouteBuilderRef && activeColorRouteBuilderRef.current) {
      ...
      history.push(
        generateColorWallPageUrl(
          ...
          params.family,
          focusedCell.current,
          fullColorName(colorMap[focusedCell.current])
        ) + (url.endsWith('family/') ? 'family/' : url.endsWith('search/') ? 'search/' : '')
      )
    }
  }
  ...
  useEffect(() => {
    if (!isEmpty(unChunkedChunks) && !isEmpty(chunkGridParams)) {
      setChunkGrid(
        makeChunkGrid(unChunkedChunks, chunkGridParams, Math.ceil(containerWidth / swatchSizeUnzoomed), !!params.family)
      )
    }
      ...
      ;((
        {
          '9': () => {
            ...
            const nextCoords = (
              e.shiftKey
                ? // generate coordinate pairs [[row, col], ...] starting from currently focused chunk going top -> bottom, right -> left
                  rangeRight(column + 1).flatMap((c) =>
                    rangeRight(c === column ? row + 1 : chunkGrid.length).map((r) => [r, c])
                  )
                : // generate coordinate pairs [[row, col], ...] starting from currently focused chunk going bottom -> top, left -> right
                  range(column, lengthOfLongestChunkRow).flatMap((c) =>
                    range(c === column ? row + 1 : 0, chunkGrid.length).map((r) => [r, c])
                  )
            )
            ...
            if (nextCoords !== undefined) {
              ...
              cellRefs.current[
                params.colorId && getCoords(nextChunk, params.colorId)[0] !== -1 ? params.colorId : nextChunk[0][0]
              ].focus()
            } else {
              ;(e.shiftKey
  ...
  useEffect(() => {
    if (!params.colorId || !gridRef.current || !chunkGrid || chunkGrid.length === 0) {
      return
    }
    ...
    const end = computeFinalScrollPosition(
      chunkGrid,
      params.colorId,
      containerWidth,
      WALL_HEIGHT,
      sectionLabels[section],
      gapsBetweenChunks ? 0.4 : 0
    )
  ...
  const chunkRenderer = ({ rowIndex: chunkRow, columnIndex: chunkColumn, key, style }) => {
    ...
    const chunkClickableProps =
      chunkClickable && params.section === kebabCase(primeColorWall)
        ? {
            onClick: () => {
              window.location.href = colorWallPageRoot?.(sectionLabels[section][chunkNum] || '')
              GA.event(
                { category: 'Color Wall', action: 'Color Family', label: sectionLabels[section][chunkNum] },
    ...
    return (
      flatten(chunk).some((cell) => cell !== undefined) && (
        <div
          key={key}
          className='color-wall-chunk'
          style={{
            ...style,
            padding: gapsBetweenChunks && !chunkMiniMap ? cellSize / 5 : 0,
            zIndex: containsBloomedCell ? 1 : 'auto'
          }}
          {...chunkClickableProps}
        >
          {sectionLabels[section] &&
            sectionLabels[section][chunkNum] !== undefined &&
            !chunkClickableProps &&
            !chunkMiniMap && (
              <div
              ...
              >
                <div
                  className={`color-wall-section-label__text ${
                    isLargeLabel ? 'color-wall-section-label__text--large' : ''
                  }`}
                      ...
                      isChipLocator ? (
                        <div className='color-swatch__chip-locator'>
                          ...
                          <div className='color-swatch__chip-locator__location'>
                            ...
                            <p className='color-swatch__chip-locator__col-row'>
      ...
      <div className='color-wall'>
        {chunkMiniMap && sectionOverride && (
          <div className='color-wall-section-label__minimap'>
            ...
            <div
              className='color-wall-section-label__minimap--image'
              style={{
                backgroundImage: `url(${
                  minimapDict[`${brandId}${leftHandDisplay ? 'LeftHand' : ''}`][sectionOverride]
                })`
              }}
            />
          </div>
        ...
        {params.colorId && (
          <button
            onClick={() => createInactiveColorRoute.current(selectedColor)}
            className={`zoom-out-btn${chunkMiniMap ? ' zoom-out-btn--minimap' : ''}`}
            ...
            <Grid
              ...
              rowHeight={({ index }): number => {
                const hasLabel: boolean =
                  sectionLabels && rowHasLabels(chunkGrid, index, sectionLabels[section]) && !chunkMiniMap
                return (
                  (getHeightOfChunkRow(chunkGrid[index]) + (gapsBetweenChunks ? 0.4 : 0)) * cellSize +
                  (hasLabel ? cellSize + (isZoomedIn ? 30 : 10) : 0)
                )
              }}
              ...
              columnWidth={({ index }) => {
                ...
                return (
                  cellSize *
                  chunkColumn.reduce(
                    (w, chunk) => Math.max(w, getLongestArrayIn2dArray(chunk) + (gapsBetweenChunks ? 0.4 : 0)),
                    0
                  )
                )
              }}
        ...
        {isChipLocator && kebabCase(params.section) === kebabCase(sectionOverride) && selectedColor ? (
          <ColorChipLocator color={selectedColor} />
        ) : null}
      </div>
    </CSSTransition>
  )
