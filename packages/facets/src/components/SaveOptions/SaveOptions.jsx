...
const SaveOptions = (props: SaveOptionsProps) => {
  ...
  const handleSave = useCallback(
    (e: SyntheticEvent) => {
      e.preventDefault()
      // Don't muddle th ealready complex save logic and give fast mask its own block
      ...
      if ((pathname === '/active' || pathname === ROUTES_ENUM.ACTIVE_PAINT_SCENE) && !toggleCompareColor) {
        if (lpColors.length !== 0) {
          const saveType =
            ACTIVE_SCENE_LABELS_ENUM.STOCK_SCENE === activeSceneLabel
              ? SAVE_OPTION.SAVE_STOCK_SCENE
              : SAVE_OPTION.SAVE_PAINT_SCENE
          if (activeSceneLabel === ACTIVE_SCENE_LABELS_ENUM.PAINT_SCENE) {
            // We handle paint scene reactively, we tell it via redux, hey it's time to globally set your data for save.
  ...
  const getFlatImage = (ctx1, ctx2) => {
    ...
    const promise = new Promise(async (resolve) => {
      ...
      loadAndDrawImage(bgData, workCtx, width - miniImgWidth, height - miniImgHeight, miniImgWidth, miniImgHeight).then(
        (ctx) => {
          const promise = new Promise((resolve) => {
      ...
      {shouldAllowFeature(featureExclusions, FEATURE_EXCLUSIONS.documentSaving) ? (
        <button onClick={handleSave}>
          <div className={saveOptionsItemsClassName}>
            <div>
              <FontAwesomeIcon title={formatMessage({ id: 'SAVE_MASKS' })} icon={['fal', 'folder']} size='2x' />
            </div>
            <div className={`${saveOptionsItemsClassName}--title`}>
              <FormattedMessage id='SAVE_MASKS' />
            </div>
          </div>
        </button>
      ) : null}
