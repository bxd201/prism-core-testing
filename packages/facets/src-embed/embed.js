...
const getPromiseFor = (facetName: string) => {
  ...
  // embed our styles
  embedTheStyles(targetedDependencies.filter((dep) => dep.match(/\.css$/)))

  // aggregate all script embed promises
  const scriptPromises = targetedDependencies
    .filter((dep) => dep.match(/\.js$/))
    .map((dep) => embedScript(dep, facetName))

  promiseFor[facetName] = Promise.all(scriptPromises).then(
    () =>
...
const prismPromise = new Promise((resolvePrism, rejectPrism) => {
  const embedQueue = new (function () {
    ...
    this.add = (...args) => {
      ...
      const attrProps: { [key: string]: string } = mapValues(Object.assign({}, el.dataset), (v) =>
        v === '' ? true : v
      )
  ...
  const allRoots = [...prismRootLegacy, ...prismRootModern]
...
function embedTheStyles(stylesheets) {
  const thisStylesheet = stylesheets.shift()
  if (thisStylesheet) {
    setTimeout(() => {
      embedStyle(thisStylesheet)
      embedTheStyles(stylesheets)
    }, 10)
  }
}
